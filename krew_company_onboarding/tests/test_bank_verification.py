...
from krew_company_onboarding.models import CompanyBankDetails
from krew_company_onboarding.services.bank_verification import (
    check_penny_drop_status,
    confirm_penny_drop,
    initiate_penny_drop,
)
from krew_company_onboarding.services.cashfree_client import CashfreePayoutError
...
_VERIFY_TARGET = "krew_company_onboarding.services.bank_verification.verify_bank_account"
_TRANSFER_TARGET = "krew_company_onboarding.services.bank_verification.initiate_bank_transfer"
_STATUS_TARGET = "krew_company_onboarding.services.bank_verification.get_transfer_status"
...
class AccountNumberEncryptionTests(TestCase):
    ...
    def test_account_number_stored_encrypted_at_rest(self):
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT account_number FROM krew_company_onboarding_companybankdetails "
                "WHERE id = %s",
    ...
    def test_masked_account_number_is_persisted_in_the_db(self):
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT masked_account_number FROM krew_company_onboarding_companybankdetails "
                "WHERE id = %s",
                [self.bank_details.pk],
            )
...
class BankDetailsReverificationTests(TestCase):
    ...
    def test_changing_account_number_clears_verified_attempt(self):
        from krew_company_onboarding.models import CompanyBankVerification
